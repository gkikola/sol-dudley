...
\Problem{12} Let $p$ be the least prime factor of $n$, where $n$ is
...
\Problem{13} True or false? If $p$ and $q$ divide $n$, and each is
...
\Problem{14} Prove that if $n$ is composite, then $2^{n-1}$ is
composite.
\begin{proof}
...
\Problem{15} Is it true that if $2^n - 1$ is composite, then $n$ is
composite?
\begin{solution}
